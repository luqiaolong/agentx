"""eval 评测评分器：Judge 协议 + AssertJudge + RubricJudge + SelfCorrectionRunner + CompositeJudge。
...
- ``CompositeJudge``：组合器，逐个执行 Judge 并异常隔离，返回 ``list[JudgeResult]``。
"""
...
from app.eval.judges.composite import CompositeJudge
