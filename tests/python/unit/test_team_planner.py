...
def test_validate_deep_agent_always_ok() -> None:
    ...
    ok, err = _validate_task(
        TeamPlanTask(agent="deep", input="do it", purpose="test"), _make_settings()
    )
    assert ok is True
    assert err == ""


def test_validate_unknown_agent_allowed() -> None:
    """未知 agent 类型校验通过，由运行时 fallback 到 code runner。"""
    ok, err = _validate_task(
        TeamPlanTask(agent="unknown", input="do it", purpose="test"), _make_settings()
    )
    assert ok is True
    assert err == ""
...
def test_validate_team_role_enabled_with_enabled_tools_ok() -> None:
    ...
    ok, err = _validate_task(
        TeamPlanTask(agent="frontend_dev", input="ui", purpose="test"), settings
    )
    assert ok is True
    assert err == ""
...
def test_validate_custom_missing_fails() -> None:
    ...
    ok, err = _validate_task(
        TeamPlanTask(agent="custom-mycoder", input="code", purpose="test"), settings
    )
    assert ok is False
    assert "不存在" in err
...
def test_validate_custom_disabled_fails() -> None:
    """自定义子代理被禁用时不可用。"""
    settings = _make_settings(custom={"mycoder": {"enabled": False, "tools": ["read_file"]}})
    ok, err = _validate_task(
        TeamPlanTask(agent="custom-mycoder", input="code", purpose="test"), settings
    )
    assert ok is False
