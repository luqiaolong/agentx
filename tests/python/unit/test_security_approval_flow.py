...
class TestResolveMaxWait:
    ...
    def test_zero_returns_absolute_max(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from app.security.approval.flow import _ABSOLUTE_MAX_WAIT, _resolve_max_wait
    ...
    def test_positive_value(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from app.security.approval.flow import _resolve_max_wait
...
class TestFullTrustSkip:
    ...
    async def test_full_trust_skips_approval(
        ...
        monkeypatch.setattr(
            "app.deepagent.approval_runner._handle_directory_extension", handle_ext_fn
        )
...
class TestHandleDirectoryExtensionBugs:
    ...
    async def test_passes_base_and_parent_thread_id(
        ...
        from app.security.approval.flow import _handle_directory_extension
        ...
        monkeypatch.setattr("app.security.approval.flow.get_settings", lambda: fake_settings)
        monkeypatch.setattr(
            "app.security.approval.flow._await_approval",
...
class TestRepeatDetectionAfterStreamResume:
    ...
    async def test_stream_fn_receives_seen_signatures_kwarg(
        ...
        async for _ in run_agent_with_approval(
            ...
            is_interrupted_fn=AsyncMock(side_effect=[True, False, False]),
            get_pending_calls_fn=AsyncMock(return_value=pending_calls),
            inject_tool_error_for_call_fn=AsyncMock(),
            inject_tool_error_messages_fn=AsyncMock(),
            max_iterations=5,
