...
覆盖：
1. ingest 成功 → partition_name=source_type，返回 id 列表
...
7. healthcheck DB 不存在 → db_not_found
8. ingest 超长文本 → on_skip 调用，被跳过文本不入库
9. search 失败 → 抛 MilvusUnavailable
"""
...
async def test_ingest_success_returns_ids_and_uses_partition():
    ...
    assert ids == [101, 102]
    # insert 调用必须带 partition_name="file"
    ...
    # data 第一组应是 kept_texts
    data = call_kwargs["data"]
    assert data[0] == ["hello world", "foo bar"]
    assert data[2] == ["file", "file"]  # source_type 列
...
async def test_search_success_returns_sorted_tuples_without_partition_name():
    ...
    with patch(
        ...
        # Milvus search 返回 [[hit, hit, ...]]；故意打乱顺序验证降序
        hits = [
...
async def test_healthcheck_auth_failed(monkeypatch):
    monkeypatch.setenv("AGENT_PY_MILVUS_USER", "wrong_user")
    monkeypatch.setenv("AGENT_PY_MILVUS_PASSWORD", "wrong_pass")
    get_settings.cache_clear()
...
# ---------------- 7. healthcheck DB 不存在 ----------------

@pytest.mark.asyncio
async def test_healthcheck_db_not_found(monkeypatch):
    monkeypatch.setenv("AGENT_PY_MILVUS_USER", "user")
    monkeypatch.setenv("AGENT_PY_MILVUS_PASSWORD", "pass")
    get_settings.cache_clear()
...
async def test_ingest_skips_too_long_text_via_on_skip():
    ...
    async def _fake_embed_texts(texts, on_skip=None):
        # 模拟 TEI 客户端契约：on_skip(text, err) — 2 个参数
    ...
    call_kwargs = client._collection.insert.call_args.kwargs
    data = call_kwargs["data"]
    # data[0] 是 text 列表，应只有 2 条（不含长文本）
    assert data[0] == ["short1", "short2"]
    # partition 仍是 file
    assert call_kwargs["partition_name"] == "file"


# ---------------- 9. search 失败抛 MilvusUnavailable ----------------
...
# ---------------- 额外：未连接时调用 ingest 抛 MilvusUnavailable ----------------
...
# ---------------- 额外：所有合法 source_type 可被 delete_by_source_type ----------------
