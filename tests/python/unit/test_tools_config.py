...
def test_tools_enabled_default_all_true() -> None:
    """不设 env，tools_enabled 返回 8 个工具全 true。"""
...
def test_make_deep_tools_all_disabled_returns_empty(
    ...
    monkeypatch.setenv(
        ...
        json.dumps({
            ...
            "web_search": False, "rag_retrieve": False,
        }),
    )
    get_settings.cache_clear()
...
def test_dangerous_tools_constant_unchanged() -> None:
    """DANGEROUS_TOOLS 常量始终包含 edit_file/write_file/shell_exec。

    常量是模块级 set，不随 tools_enabled 变化。
    即使工具被禁用，常量本身不变（运行时危险集合通过交集计算）。
    """
    assert DANGEROUS_TOOLS == {"edit_file", "write_file", "shell_exec"}
...
def test_runtime_dangerous_all_enabled() -> None:
    ...
    # 工具集含 edit_file/write_file，不含 shell_exec
    assert runtime_dangerous == {"edit_file", "write_file"}
