...
# ============================================================
# 5. readonly streak / 重复调用 循环保护
# ============================================================


class TestSupervisorReadonlyStreakProtection:
    """Supervisor readonly streak 超限 / 重复调用检测 循环保护测试。
    ...
    async def test_readonly_streak_triggers_force_answer_not_error(self) -> None:
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._make_deep_tools", return_value=[]))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._load_mcp_tools", new_callable=AsyncMock, return_value=([], set())))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor.make_delegation_tools", return_value=[]))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor.build_work_supervisor", new_callable=AsyncMock))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._sanitize_message_history", side_effect=lambda x, y: x))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._inject_tool_error_messages", new_callable=AsyncMock))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._inject_tool_error_for_call", new_callable=AsyncMock))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor.is_paused", new_callable=AsyncMock, return_value=False))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor.get_sandbox", return_value=MagicMock()))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._handle_directory_extension", new_callable=AsyncMock, return_value=SimpleNamespace(events=[], denied=False, timed_out=False)))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._is_interrupted", new_callable=AsyncMock, side_effect=interrupted_values))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._get_pending_tool_calls", new_callable=AsyncMock, side_effect=pending_values))
            stack.enter_context(patch("app.agents.supervisor.work_supervisor._stream_agent_events", side_effect=_stream_side_effect))

            events = []
            async for sse in run_work_supervisor("项目技术栈", "test-streak"):
                events.append(sse)
    ...
    async def test_duplicate_tool_calls_triggers_force_answer_immediately(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            events = []
            async for sse in run_work_supervisor("列出目录", "test-dup"):
                events.append(sse)
    ...
    async def test_normal_flow_below_threshold_no_force_answer(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            events = []
            async for sse in run_work_supervisor("简单问题", "test-normal"):
                events.append(sse)
