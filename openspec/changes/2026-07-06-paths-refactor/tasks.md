...
- [ ] 验证 `utils/prompts.py` 的 `resolve_system_prompt` 与 `graph.py` 版本完全一致（已确认一致）

## Phase 2: 拆 graph.py 内联代码到 chat/ 和 subagents/

- [ ] 新建 `backend/app/chat/__init__.py`
  - 导出 `run_chat_path`
- [ ] 新建 `backend/app/chat/run.py`
...
- [ ] 新建 `backend/app/subagents/dispatch.py`
...
- [ ] 新建 `backend/app/team/__init__.py`
  - 导出 `run_team_path`, `Blackboard`, `TeamPlanTask`, `TeamSubtaskResult`
- [ ] 新建 `backend/app/team/orchestrator.py`
...
- [ ] 新建 `backend/app/memory/profile_extractor.py`
...
- [ ] 修改 `deep/agent.py` 的 `run_deep_path` 结尾 T10 部分
  - 改为：
    ```python
...
- [ ] 不修改 `memory/__init__.py`（`profile_extractor` 通过完整路径 import）
...
- [ ] 删除 `backend/app/paths/__init__.py`
- [ ] 删除 `backend/app/paths/deep_path.py`
- [ ] 删除 `backend/app/paths/team_path.py`
- [ ] 删除 `backend/app/paths/` 目录
...
- [ ] `tests/python/unit/test_router_graph.py:23`
...
- [ ] `tests/python/unit/test_custom_subagents.py:26`
  - `from app.router.graph import _select_subagent` → `from app.subagents.dispatch import select_subagent`
- [ ] `tests/python/unit/test_subagents_config.py:20`
  - `from app.router.graph import _select_subagent` → `from app.subagents.dispatch import select_subagent`
- [ ] `tests/python/unit/test_tools_config.py:17`
  - `from app.paths.deep_path import (...)` → `from app.deep.agent import (...)`
- [ ] `tests/python/unit/test_memory_profile.py`（4 处：L322, L344, L360, L376）
  - `from app.paths.deep_path import _extract_profile_via_llm` → `from app.memory.profile_extractor import extract_profile_via_llm`
  - 调用处 `_extract_profile_via_llm(...)` → `extract_profile_via_llm(...)`
- [ ] `tests/python/unit/test_team_path.py:21`
  - `from app.paths.team_path import (...)` → `from app.team.orchestrator import (...)`（同函数名）
- [ ] `tests/python/unit/test_skills_api.py:15` — 不变（`_parse_skill_tag` 保留在 graph.py）
...
- [ ] `tests/python/unit/test_memory_profile.py`
  - `app.paths.deep_path.get_chat_model`（L332, L350, L366, L385）→ `app.memory.profile_extractor.get_chat_model`
- [ ] `tests/python/unit/test_chat_control_api.py:157` — 不变（`app.router.graph.classify_message` 仍有效）

### 9.3 运行测试

- [ ] 运行全量测试：`uv run pytest tests/python/unit -m "not integration"`
- [ ] 确认测试全部通过（423+ 测试用例）

## Phase 10: 文档更新

- [ ] 更新 `AGENTS.md` §11 文件地图
...
- [ ] 更新 `AGENTS.md` §12 Router 三路径表格中的文件链接
  - 路径 A → `chat/run.py`
  - 路径 B → `subagents/dispatch.py`
  - 路径 C → `deep/agent.py`
  - 新增路径 D → `team/orchestrator.py`
- [ ] 更新 `AGENTS.md` §14.5 路径导入循环说明（循环已消除，deep 不再依赖 router.graph）
