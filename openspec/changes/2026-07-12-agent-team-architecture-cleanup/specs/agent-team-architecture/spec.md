...
Team 路径子任务节点从 6 个复制粘贴函数收敛为单一 `_run_subtask_node` + `_NODE_DISPATCH` 派发表，
所有 agent 类型共享相同的 abort/delegation/todo/runner/CancelledError 流程，差异由 `SubtaskConfig`
描述。未知 agent 类型经 `_DEFAULT_CONFIG` 统一 emit `delegation` 后返回失败。
...
`_run_subtask_node` 在统一 6 个节点函数后，必须保留 Phase 1 引入的 `subtask_timeout` 超时保护
与 `max_parallel` 并发限流。超时保护由 `_run_subtask_stream` 内部的 `asyncio.wait_for(timeout=subtask_timeout)`
自动生效；并发限流由 `_run_subtask_node` 在调用 runner 前获取 `state["team_semaphore"]` 实现。
...
`in_progress` 状态由 `_merge_todos` reducer 持久化，删除模块级全局 `_in_progress_tasks`。
reducer 不变量：`in_progress` 一旦设置，只能由显式 `completed`/`error` 覆盖，不因 values-mode
归并回退为 `pending`。
...
删除从未被读取的 `TeamState.subtask_results`、从未被填充的 `RouterState.team_blackboard` 与
`Blackboard.meta`，`Blackboard` dataclass 内联进 `_run_aggregator` 入参。`clean_todos`/`tasks`
双列表合并为单一 `tasks` 数据源 + 派生函数 `tasks_to_display_todos`。
...
`/reset` 端点在删除父 thread checkpoint 后，枚举并删除所有 `{thread_id}-team-*` 前缀的子 thread
checkpoint，避免 `data/agentx.db` 无限膨胀。child_thread_id UUID 化防止 retry 撞 stale 状态。
...
静默的 `except Exception: pass` 收窄为窄异常类型，非中断异常经 `logger.warning` 可观测。
`_inherit_workspace` 授权失败返回失败结果而非继续未授权执行。
...
子任务执行期间，abort 信号在 5 秒内被响应，即使 LLM 处于长流式调用中。
...
`_quality_gate` 恢复对所有 findings 全等的拒绝逻辑，避免 4 个相同非截断结果被无意义汇总。
...
简单任务降级（短消息/问候/翻译）不 emit `team_done`，因无对应 `team_init`。降级等价于单 agent
...
`_looks_like_dangerous_task` 用编译正则匹配，ASCII 关键词带 `\b` 词边界，CJK 关键词用子串。
关键词模式与 aggregator 共享 helper，消除两处分叉。
...
新增 `backend/app/utils/text.py` 提供 `compile_keyword_patterns(keywords)` 与 `matches_any(text, patterns)`，
planner 与 aggregator 共用以消除关键词匹配逻辑分叉。
