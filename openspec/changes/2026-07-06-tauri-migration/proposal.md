...
| 凭证加密 | safeStorage → stronghold + OS keyring |
...
- `electron-store` + `safeStorage` → `tauri-plugin-store` + `tauri-plugin-stronghold`
...
- `electron-store` 凭证数据 → stronghold 格式（迁移脚本）
...
- 凭证加密跨平台行为统一（stronghold 抽象 OS keychain）
...
| stronghold / tauri-plugin-store 跨平台差异 | 中 | 选用 stable API + 在 Windows 上做完整冒烟 |
...
| D3 | stronghold 加密 + tauri-plugin-store 持久化 | stronghold 提供跨平台 OS keyring 抽象 |
...
| D7 | 凭证数据迁移脚本：读取 electron-store JSON → 写入 stronghold 格式 | 老用户不需重配 LLM/Milvus key |
