...
> 状态：已合并到 master（2026-07-09，commit 501d7c1）。所有任务项已完成。

## 阶段一：观测基础设施（SqliteObservationSink + Callback）

- [x] T1.1 新建 `backend/app/observability/observation.py`：
...
- [x] T1.2 修改 `backend/app/main.py` lifespan：
  - 启动时调 `get_observation_sink()` 单例
  - shutdown 时 `sink.close()`
- [x] T1.3 新建 `tests/python/unit/test_observation_sink.py`：
  - 性能测试：append 1000 event < 200ms（隔离运行 ~140ms，并行套件放宽到 2000ms）
  - 崩溃恢复：异常退出后重连数据不丢
...
- [x] T3.1 修改 `backend/app/router/graph.py::run_router`：
...
- [x] T3.4 修改 `backend/app/api/chat.py::chat_approve`（函数定义在 L155，L154 是装饰器）：
  - 按 `thread_id` 找到最近 `approval_request` 事件，回填 `approval_decision` / `auto_approved`
- [x] T3.5 新建 `tests/python/unit/test_observation_callback.py`：
...
- [x] T4.1 新建 `backend/app/api/observation.py`：
...
- [x] T4.2 修改 `backend/app/api/__init__.py`：
  - `register_observation_routes(app)` 注册 5 个端点
- [x] T4.3 新建 `backend/app/observability/feedback.py`：
...
- [x] T4.4 新建 `frontend/renderer/components/chat/MessageFeedback.tsx`：
  - 👍 按钮（点击直接写 thumb_up）
  - 👎 按钮（点击展开 popover：分类下拉 + 评论输入框）
  - 复用 [usePopover.ts](file:///d:/java/agentprojects/agentx/frontend/renderer/components/ui/hooks/usePopover.ts) 状态管理
- [x] T4.5 修改 `frontend/renderer/components/chat/AssistantMessageParts.tsx`：
  ...
  - hover 态浮现，默认不占空间
- [x] T4.6 修改 `frontend/renderer/lib/api/chat.ts`：
  - 新增 `apiPost('/api/observation/feedback', { run_id, kind, score?, comment?, categories? })` 客户端
- [x] T4.7 修改 `frontend/shared/api-types.ts`：
  - 补充 `FeedbackKind` / `FeedbackRequest` / `ObservationRun` / `ObservationEvent` 类型
  - SSE 事件 data 字段类型扩展 `& { _tid?: string }`
- [x] T4.8 新建 `tests/python/unit/test_observation_api.py`：
  - 5 端点契约（请求/响应 schema）
  - 权限：feedback 写入限同 thread_id 用户
- [x] T4.9 新建 `tests/python/unit/test_feedback_implicit.py`：
  - 隐式信号 3 种类型
- [x] T4.10 新建 `tests/renderer/MessageFeedback.test.tsx`：
  - 👍 按钮点击 → API 调用
...
- [x] T5.1 修改 `backend/app/eval/cli.py`：
...
- [x] T5.2 新建 `tests/python/unit/test_eval_feedback_export.py`：
...
- [x] T6.1 更新 `AGENTS.md` §3 反面清单：
  - 明确"自研 trace 协议"的例外（已用 LangSmith SDK + LangChain Callback，R18）
  - 明确"自研评估器"的例外（已用 deepagents `RubricMiddleware`）
- [x] T6.2 更新 `.env.example`：
...
- [x] FR-11.1 TTL reaper 周期任务（Phase 3 review 发现 lifespan 仅 startup 清一次，
      缺 6h 周期 reaper）：
  - `start_observation_reaper(interval_hours=6.0)` 后台协程
  - lifespan 启动 + shutdown 与审批 reaper 一起 cancel
  - 2 个新测试覆盖（周期 + 异常隔离）
...
- `backend/app/main.py` — lifespan 注入 + reaper
- `backend/app/api/chat.py` — _event_generator + chat_approve
- `backend/app/api/__init__.py` — 注册路由
- `backend/app/api/schemas.py` — ApproveRequest/AbortRequest + run_id
- `backend/app/router/graph.py` — prompt + snapshot 写入点
- `backend/app/deep/streaming.py` — 事件 append 点
- `backend/app/cli/app.py` — eval export-feedback subparser
- `backend/app/eval/cli.py` — export-feedback 子命令
- `pyproject.toml` — 新增 `langsmith>=0.1.0`
- `frontend/renderer/components/chat/AssistantMessageParts.tsx` — 集成反馈按钮
- `frontend/renderer/hooks/useChatStream.ts` — trace_id 同步
- `frontend/renderer/components/chat/ChatView.tsx` — 预填 traceId
- `frontend/renderer/stores/chat/index.ts` — ChatMessage.traceId + setMessageTraceId
- `frontend/renderer/lib/api/chat.ts` — feedback API client
- `frontend/shared/api-types.ts` — FeedbackKind / ObservationRun / _tid 类型
- `AGENTS.md` — §3 R18 + §11 文件地图
- `.env.example` — LangSmith + observation TTL 文档
- `README.md` — 观测 / 反馈 / 复盘 章节
- `uv.lock` — pydantic/langsmith 依赖锁定

## 规模判定

- 涉及文件数: 31（12 新建 + 19 修改）[超出原 estimate 24，因 P4 TTL reaper + 前端 _tid 字段扩展]
- 涉及模块数: 6（backend/observability + backend/api + backend/router + backend/deep + backend/eval + frontend）
...
|----|---------|---------|---------|------|
| T1 | 观测基础设施（sink + callback）| observation.py / main.py / test_observation_sink.py | 4 表建表 + 1000 event < 200ms + 崩溃恢复 | ✅ commit a1662d6 |
| T2 | LangSmith 真实 SDK 接入 | langsmith.py / langsmith_dual.py / pyproject.toml / .env.example | 双写 + 降级 + `langsmith>=0.1.0` 声明 | ✅ commit 05d9d83 |
| T3 | 五维度串联 | router/graph.py / deep/streaming.py / api/chat.py / test_observation_callback.py | prompt 落盘 + state snapshot 3 点 + event append | ✅ commit 5225f10 |
| T4 | 用户反馈端点 + 前端 | api/observation.py / feedback.py / MessageFeedback.tsx / MessageBubble.tsx / 共享类型 / 4 个测试 | 5 端点契约 + 👍/👎 UI + 隐式 3 信号 | ✅ commit b99d2cd |
| T5 | 反馈 → 评测闭环 | eval/cli.py / test_eval_feedback_export.py | export-feedback 子命令 + YAML 转换 + replay 跑通 | ✅ commit 26ff927 |
| T6 | 文档 + AGENTS 更新 | AGENTS.md / .env.example / README.md | 反面清单例外说明 + LangSmith 文档 + 观测章节 | ✅ commit afa8f6d |
| P4 | FR-11.1 TTL reaper | observation.py / main.py / test_observation_sink.py | 6h 周期后台协程 + 异常隔离 | ✅ commit 4e6a8bd |
| P5 | Merge to master | - | squash merge 无冲突（streaming.py 冲突预 stash） | ✅ commit 501d7c1 |

## 复盘验证

```bash
# 单元测试（55 个新增）
cd .worktrees/agent-observation-store
uv run pytest tests/python/unit/test_observation_sink.py \
              tests/python/unit/test_observation_callback.py \
              tests/python/unit/test_langsmith_dual.py \
              tests/python/unit/test_observation_api.py \
              tests/python/unit/test_feedback_implicit.py \
              tests/python/unit/test_eval_feedback_export.py -v
# → 55 passed

# 前端测试（10 个新增）
npx vitest run tests/renderer/MessageFeedback.test.tsx
# → 10 passed

# Backend (本 PR 涉及子目录)
uv run ruff check backend/app/observability/ backend/app/api/
# → All checks passed
