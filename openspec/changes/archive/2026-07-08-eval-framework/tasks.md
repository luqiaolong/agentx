...
- [x] T1.1 `backend/app/eval/models.py`:
  - 删除 `LlmJudgeConfig` 类
...
- [x] T1.2 更新 `tests/python/unit/test_eval_models.py`:
...
- [x] T2.1 新建 `backend/app/eval/judges/rubric_judge.py`:
...
- [x] T2.2 删除 `backend/app/eval/judges/llm_judge.py`
- [x] T2.3 更新 `backend/app/eval/judges/__init__.py`:
  - 移除 `LlmJudge` 导出
  - 新增 `RubricJudge` 导出
- [x] T2.4 新建 `tests/python/unit/test_eval_rubric_judge.py`:
...
- [x] T3.1 扩展 `backend/app/eval/mocks/llm.py` MockChatModel:
...
- [x] T3.2 新建 `backend/app/eval/judges/self_correction.py`:
...
- [x] T3.3 更新 `backend/app/eval/judges/__init__.py`:
  - 新增 `SelfCorrectionRunner` 导出
- [x] T3.4 新建 `tests/python/unit/test_eval_self_correction.py`:
  - 测试 satisfied 一次通过
...
- [x] T4.1 修改 `backend/app/eval/runner.py`:
...
- [x] T4.2 更新 `tests/python/unit/test_eval_runner.py`:
...
- [x] T5.2 修改 `backend/app/cli.py`:
  - eval parser 的 `--no-llm-judge` → `--no-rubric`
- [x] T5.3 修改 `backend/app/eval/pytest_plugin.py`:
  - mock 模式：`RubricJudge(no_rubric=True)`
  - live 模式：`RubricJudge()`
- [x] T5.4 更新 `tests/python/unit/test_eval_cli.py`:
  - `--no-llm-judge` → `--no-rubric`
- [x] T5.5 更新 `tests/python/unit/test_eval_pytest_plugin.py`:
  - 适配 RubricJudge

## 阶段六：YAML 评测集

- [x] T6.1 创建 `tests/eval/suites/smoke.yaml`（5-8 个 case，含 rubric + self_correct）
- [x] T6.2 创建 `tests/eval/suites/router.yaml`（3 个 case，含 rubric）
- [x] T6.3 创建 `tests/eval/suites/tools.yaml`（4 个 case，含 rubric）
- [x] T6.4 创建 `tests/eval/suites/team.yaml`（3 个 case，含 rubric）
- [x] T6.5 创建 `tests/eval/suites/cli.yaml`（4 个 case，含 rubric）
- [x] T6.6 新增/更新 mock fixtures（`backend/app/eval/mocks/fixtures/`）支持 rubric grader 响应:
...
- [x] T7.1 创建 `scripts/eval-gate.ps1`:
...
- [x] T8.1 smoke suite 离线跑通：`agentx eval run --suite smoke`（无 API key，30 秒内完成）
- [x] T8.2 现有测试不破坏：`pytest tests/python/unit/ -q`
- [x] T8.3 eval 专项测试：`pytest tests/python/unit/test_eval_*.py -q`
- [x] T8.4 pytest 插件集成：`pytest tests/eval/suites/ -q`（YAML 自动收集）
- [x] T8.5 门禁脚本可用：`.\scripts\eval-gate.ps1`
