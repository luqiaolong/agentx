...
本地优先的个人助理桌面应用：Tauri 壳 + React 渲染层 + FastAPI/Python 后端 + LangGraph
...
├── src-tauri/                  ← Rust 主进程（替代 frontend/main + preload）
│   ├── src/
│   │   ├── lib.rs              ← run() 入口 + setup() 启动 Python + 注册 54 命令
│   │   ├── backend/            ← PythonHandle（spawn + 健康握手 + 崩溃退避）
│   │   ├── commands/           ← 54 个 #[tauri::command]（settings/dialog/shell/window/clipboard/notify/logs/app/git）
│   │   ├── store/              ← tauri-plugin-store 封装（enc:/plain: 凭证）
...
`backend/app/main.py::_event_generator` 与 `frontend/renderer/lib/api/chat.ts::streamChat`
共同实现。事件类型（`event` 字段）：
...
- 后端 `Settings` 用 `env_prefix="AGENTX_"` + `env_file=None`，**禁止**从 `.env` 读凭证。
- 凭证（LLM key / Milvus user/password）由 Rust 主进程从 `tauri-plugin-store`（`enc:`/`plain:`
  前缀格式）→ 通过 `tokio::process::Command::env()` 注入进程环境。
...
- 后端 8123 端口由 `src-tauri/src/backend/handle.rs::PythonHandle::start` 启动
  （`uv run python -m app.main`，uv 缺失则回退 `python -m app.main`）。
- 崩溃退避：指数 1s/2s/4s 最多 3 次 → `giving_up` 状态由前端遮罩兜底。
...
- 沙箱授权目录通过 `POST /api/sandbox/authorize` 显式开启（renderer 直连 HTTP，**不**走 invoke）。
- 系统关键目录黑名单（Windows / Unix）在 `src-tauri/src/commands/dialog.rs::save_dropped_file`。
- `RouterState.authorized_dirs` 随 checkpoint 持久化，实现跨会话恢复。
...
- `@` → `frontend/renderer`
- 见 [vite.config.ts](file:///d:/java/agentprojects/agentx/vite.config.ts) + [tsconfig.web.json](file:///d:/java/agentprojects/agentx/tsconfig.web.json)。
...
等 `AGENTX_*` 环境变量读取配置，由 `src-tauri/src/backend/env.rs::build_env` 注入。

**配置变更即时生效**（无需重启后端）：
- Renderer 保存配置 → `invoke("settings_set_*")` → tauri-plugin-store → `invoke("app_reload_backend_config")`
...
不重启 Tauri 窗口）。全量重启 Tauri（`invoke("app_restart")`）仅用于 ErrorBoundary 渲染错误恢复。
